...
Request.prototype.end = function (s) {
    ...
    for (var i = 0; i < keys.length; i++) {
        var key = keys[i];
        var headerName = this._headerNames[key];
        var value = this._headers[key];
        if (isArray(value)) {
            for (var j = 0; j < value.length; j++) {
                this.xhr.setRequestHeader(headerName, value[j]);
            }
        }
        else this.xhr.setRequestHeader(headerName, value)
    }
